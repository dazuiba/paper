load "image_parser.rb"
class Admin::PaperFlowsController < ApplicationController
  # GET /paper_flows
  # GET /paper_flows.xml
  def index
    @paper_flows = PaperFlow.paginate(:page=>params[:page]||1)
  ...
  def new
    ...
    begin
      if params["commit"] == "解析"
        info = params[:parse_info]
        @parser = BatchImageParser.new(info[:url_pattern], "", info[:value], :css_path=>info[:css_path])
      end
